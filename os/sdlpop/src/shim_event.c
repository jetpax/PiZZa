 ...
 * pop_event_submit() -- SDLPoP's own SDL_PushEvent (timer callbacks,
...
LOG_MODULE_REGISTER(pop_event, CONFIG_SDLPOP_LOG_LEVEL);

#define POP_EVENT_QUEUE_LEN 64

static SDL_Event queue[POP_EVENT_QUEUE_LEN];
...
{
	return (pop_event_submit(event) == 0) ? 1 : 0;
}
...
int SDL_PollEvent(SDL_Event *event)
{
	...
	q_head = (q_head + 1) % POP_EVENT_QUEUE_LEN;
	q_count--;
	k_spin_unlock(&q_lock, key);
	return 1;
}
