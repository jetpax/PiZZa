 ...
 *
 * PiZZa SDL2 shim -- minimal SDL2 API surface for retro games on Zephyr.
 *
 * This is NOT a general SDL2 port. It declares exactly the symbols the
 * ported games reference (SDLPoP + sdl2-doom, grep-verified) and no
...
enum {
	...
	/* 32-bit XRGB (0x00RRGGBB); sdl2-doom's i_video texture format.
...
/* Keycodes (event.key.keysym.sym). SDL2 scheme: printable keys are
 * their ASCII code, others are scancode | SDLK_SCANCODE_MASK. Doom's
 * i_input.c switches on these; the event producer derives keysym.sym
 * from keysym.scancode so every input source fills it for free.
 */
...
/* Joystick hat positions (i_joystick.c reads SDL_JoystickGetHat). */
...
/* sdl2-doom's i_joystick.c calls this inconsistently (once with an
 * opened SDL_Joystick*, once with a device index); both paths are dead
