 ...
 *
 * PiZZa SDL2 shim -- software surfaces, palettes and blits.
 *
 * Promoted from apps/SDLPoP (its surface universe: 8bpp indexed
 * sprites with palette + colorkey, 24bpp composition, 32bpp overlay
 * helpers) now that a second consumer (sokoban's render engine + TTF
 * atlas) needs surfaces. Pixel memory is always CPU-visible;
 * Lock/Unlock are no-ops. Dual-target: builds under Zephyr and in the
 * host tests (s2s_compat.h).
 */
		    ...
		    SDL_Surface *dst, SDL_Rect *dstrect)
{
	...
	/* fast path: same layout, no colorkey, no blending. The palette
	 * compare spans the full backing array (upstream compared
	 * sizeof(pointer) -- 2 entries -- a latent bug this promotion
	 * caught).
	 */
	if (blend != SDL_BLENDMODE_BLEND && !ps->has_colorkey && sbpp == dbpp &&
	    (sbpp != 1 || memcmp(ps->colors, pd->colors,
				 sizeof(ps->colors)) == 0)) {
