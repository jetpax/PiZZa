 ...
 * (CONFIG_SDL2SHIM_SCRIPTED_INPUT).
 *
...
LOG_MODULE_REGISTER(s2s_scripted, CONFIG_SDL2SHIM_LOG_LEVEL);
...
/* Under CONFIG_SDL2SHIM_SIM_TUNING the game boots straight into level 1
...
void s2s_scripted_input_start(void)
{
	...
	k_thread_name_set(&scripted_thread, "s2s_scripted");
}
