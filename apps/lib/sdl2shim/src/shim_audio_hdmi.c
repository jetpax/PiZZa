 ...
 *
 * PiZZa SDL2 shim -- HDMI audio backend (CONFIG_SDL2SHIM_AUDIO_HDMI).
 ...
 *
 * The M2 test tone (CONFIG_SDL2SHIM_AUDIO_HDMI_TEST_TONE) reuses this
...
LOG_MODULE_REGISTER(s2s_audio, CONFIG_SDL2SHIM_LOG_LEVEL);
...
#ifdef CONFIG_SDL2SHIM_AUDIO_HDMI_TEST_TONE
...
static int hdmi_open_common(rs48_source_fn source, void *source_user)
{
	...
	if (!device_is_ready(dma_dev)) {
		s2s_set_error("audio: DMA controller not ready");
		return -1;
	}

	err = hdmi_audio_hw_init();
	if (err != 0) {
		s2s_set_error("audio: HDMI audio unavailable (%d)", err);
		return -1;
	}

	st.dma_chan = dma_request_channel(dma_dev, NULL);
	if (st.dma_chan < 0) {
		s2s_set_error("audio: no free DMA channel");
		return -1;
	}
	...
	if (err != 0) {
		s2s_set_error("audio: dma_config failed (%d)", err);
		goto fail_release;
	}
	...
	if (err != 0) {
		s2s_set_error("audio: dma_start failed (%d)", err);
...
{
	if (st.opened) {
		s2s_set_error("audio: already open%s",
	...
	if (desired == NULL || desired->freq != 44100 ||
		...
		s2s_set_error("audio: backend is fixed 44100/S16/2ch/callback");
		return -1;
	}
...
#endif /* CONFIG_SDL2SHIM_AUDIO_HDMI_TEST_TONE */
