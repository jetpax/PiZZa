...
static int s2s_usb_start(void)
{
	struct usbd_context *usbd = sample_usbd_init_device(NULL);

	if (usbd == NULL) {
		printk("[sdl2shim] sample_usbd_init_device failed\n");
		return -ENODEV;
	}
	if (!usbd_can_detect_vbus(usbd)) {
		...
		if (err) {
			printk("[sdl2shim] usbd_enable failed: %d\n", err);
			return err;
		}
	}
	return 0;
...
SYS_INIT(s2s_usb_start, APPLICATION, CONFIG_APPLICATION_INIT_PRIORITY);
