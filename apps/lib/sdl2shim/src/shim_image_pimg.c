 ...
 *
 * Promoted from apps/SDLPoP. Each consumer's asset packer pre-converts
 * every PNG to a raw PIMG blob at build time (indexed pixels + RGBA
 * palette preserved), so "loading an image" is a header parse + row
 * copy -- no PNG decoder on the device. Format defined in the packer
 * (SDLPoP's tools/pack_assets.py is the reference).
 *
 * IMG_SavePNG is screenshot-only in every consumer -> permanent stub.
...
/* Truecolor PIMG channel masks (LE). */
#define PIMG_RMSK24 0x000000FF
#define PIMG_GMSK24 0x0000FF00
#define PIMG_BMSK24 0x00FF0000
#define PIMG_RMSK32 0x00FF0000
#define PIMG_GMSK32 0x0000FF00
#define PIMG_BMSK32 0x000000FF
#define PIMG_AMSK32 0xFF000000
...
static SDL_Surface *pimg_to_surface(const uint8_t *blob, size_t size)
{
	...
	case PIMG_RGB24:
		surf = SDL_CreateRGBSurface(0, w, h, 24,
					    PIMG_RMSK24, PIMG_GMSK24, PIMG_BMSK24, 0);
		row_bytes = (size_t)w * 3;
		break;
	case PIMG_ARGB32:
		surf = SDL_CreateRGBSurface(0, w, h, 32,
					    PIMG_RMSK32, PIMG_GMSK32, PIMG_BMSK32, PIMG_AMSK32);
...
{
	/* Every "decoder" the pack pipeline supports is always available;
	 * echo the request so init-flag equality checks pass.
	 */
	return flags;
}
...
{
	static const SDL_version v = { SDL_IMAGE_MAJOR_VERSION,
