...
static struct pop_timer timers[POP_MAX_TIMERS];
...
static void pop_timer_expiry(struct k_timer *ktimer)
{
	struct pop_timer *t = CONTAINER_OF(ktimer, struct pop_timer, ktimer);
...
SDL_TimerID SDL_AddTimer(Uint32 interval, SDL_TimerCallback callback, void *param)
{
	if (callback == NULL || interval == 0) {
		pop_set_error("AddTimer: bad args");
		return 0;
	}

	for (int i = 0; i < POP_MAX_TIMERS; i++) {
		if (timers[i].in_use) {
			continue;
		}

		struct pop_timer *t = &timers[i];

		t->callback = callback;
		t->param = param;
		t->interval_ms = interval;
		t->in_use = true;
		k_timer_init(&t->ktimer, pop_timer_expiry, NULL);
		k_timer_start(&t->ktimer, K_MSEC(interval), K_MSEC(interval));
		return i + 1;   /* SDL_TimerID 0 = failure */
	}

	pop_set_error("AddTimer: pool exhausted");
	return 0;
}

SDL_bool SDL_RemoveTimer(SDL_TimerID id)
{
	if (id < 1 || id > POP_MAX_TIMERS || !timers[id - 1].in_use) {
		return SDL_FALSE;
	}
