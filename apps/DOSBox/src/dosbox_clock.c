 ...
 *
 * PiZZa DOSBox-X -- ARM clock bump (rpi_zero_2w).
 *
 ...
 * cpufreq governor. A DOS emulator is CPU-bound (the dynrec emulating
 * 386 protected mode), so at the idle clock DOOM runs ~60% of a 1 GHz
 * pace -- slow-motion. A boot-time SET_CLOCK_RATE with
 * skip_setting_turbo=0 pins the A53 at its rated 1.0 GHz (measured ~1.6x
 * on the Jet port). Opt-in per-app, per [[jet-arm-1ghz-bump]].
 */
...
void dbx_bump_arm_clock(void)
{
	...
	printk("[dbx] ARM clock: %u -> %u Hz (SET err=%d)\n",
