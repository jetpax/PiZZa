 ...
 *
 * PiZZa DOSBox-X -- display present backend (CONFIG_DOSBOX_PRESENT_DISPLAY).
 *
 * DOSBox-X changes video mode at runtime (VGA text 720x400, the DOSBox
 * logo 640x480, DOOM's 640x400, ...), but the Zephyr display API has a
 * fixed resolution set at probe. So the VC framebuffer is pinned at a
 * fixed 640x400 virtual mode (render-width/height in the overlay) and
 * each incoming frame is nearest-neighbor scaled into it; the VideoCore
 * HVS then scales 640x400 to the monitor's native mode -- the same
 * proven trick as the Jet / wipeout / SDLPoP / Doom ports.
 *
 * 640x400 is chosen so DOOM (which DOSBox presents at 640x400) lands
 * 1:1, no resampling on the showcase path. DOSBox pixels are already
 * 0x00RRGGBB; the scanout wants 0xFFRRGGBB, so alpha is forced opaque.
 */
...
void s2s_present_frame(const void *xrgb_pixels, int pitch)
{
	...
	for (int y = 0; y < FB_H; y++) {
		...
		if (src_w == FB_W) {
			/* DOOM fast path: 1:1 columns, just force alpha */
			for (int x = 0; x < FB_W; x++) {
